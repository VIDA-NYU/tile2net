...
) -> Callable[[Callable[..., object]], Callable[..., Future]]:
    def submit(func: Callable[..., object], /, **kwargs) -> Future:
        # ensure we forward ALL kwargs (filename AND array, params, etc.)
        if path_key not in kwargs:
            # try a couple of tolerant fallbacks
            for k in ("path", "outfile", "out_path"):
                if k in kwargs:
                    path = kwargs[k]
                    path_key_local = k
                    break
            else:
                raise KeyError(f"submit: missing path kwarg; expected '{path_key}'")

            # rebind to the discovered key so downstream code is uniform
            kwargs[path_key] = path
            del kwargs[path_key_local]

        gate.acquire()  # hard backpressure

        def job() -> None:
            try:
                p = Path(kwargs[path_key])
                fd, tmp = tempfile.mkstemp(prefix=".tmp_", dir=str(p.parent))
                os.close(fd)
                try:
                    call_kwargs = dict(kwargs)
                    call_kwargs[path_key] = tmp
                    # fully forward ALL args (array, params, etc.)
                    func(**call_kwargs)
                    os.replace(tmp, p)
                except Exception:
                    try:
...
def imwrite_atomic(
    ...
    path = Path(path)
    tmp = path.with_name(f".tmp_{uuid.uuid4().hex}_{path.name}")
    # rely on the image writer to close its fd on return; no per-tile fsync
    ok = imwrite_func(str(tmp), array)
    if ok is False:  # e.g., cv2.imwrite returns False on failure
        try:
            tmp.unlink(missing_ok=True)
        finally:
            raise RuntimeError(f"imwrite failed for {path}")
    os.replace(tmp, path)
    ...
    def predict(
        ...
        with grid.cfg as cfg:
            ...
            with ExitStack() as stack, \
                ...
                threads = ThreadPoolExecutor(max_workers=writer_workers)
                gate = BoundedSemaphore(max_inflight)
                prev_futures: list[Future] = []

                # def submit( func, **name2path):
                #     gate.acquire()
                #
                #     def atomic( func, **name2path):
                #         name, path = next(iter(name2path.items()))
                #         path = Path(path)
                #         try:
                ...
                #
                #     try:
                #         future = threads.submit(atomic, func=func, **name2path)
                #     except Exception:
                #         gate.release()
                #         raise
                #     return future

                submit = _make_submitter(threads, gate, path_key='filename')

                try:
                    for n, batch in enumerate(loader):
                        # drain previous batch (keeps latency bounded)
                        if prev_futures:
                            done, _ = wait(prev_futures)
                            for fut in done:
                                try:
                        ...
                        # lightweight periodic stats only (no malloc_trim here)
                        proc = psutil.Process(os.getpid())
                        rss_gb = proc.memory_info().rss / (1024 ** 3)
                        cuda = (
                            torch.cuda.memory_reserved() / (1024 ** 3),
                        ...
                        logger.debug(
                            f'Batch {n} | RSS={rss_gb:.2f} GB | CUDA reserved/alloc={cuda[0]:.2f}/{cuda[1]:.2f} GB'
                        )
