...
class Local(Source):
    ...
    def template(self) -> str:
        """Template string with {x}, {y}, {z} placeholders for tile coordinates."""
    ...
    def from_str(cls, value: str) -> Self:
        ...
        instance.template = ''.join(parts) + ext

        instance.root = parts[0].rsplit('/', 1)[0]
        if os.sep in parts[0]:
            instance.root = parts[0].rsplit(os.sep, 1)[0]

        instance.suffix = os.path.relpath(instance.template, instance.root)
