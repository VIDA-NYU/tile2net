...
class Local(
    ...
    def glob(self, itile: str = None) -> pd.Series:
        """
        ...
        Args:
            itile: Optional template string. If provided, the index of the returned Series
                   ...
                   from the filepaths (e.g., 'new_{i}.png'). The index name will be 'itile'.
        ...
        # 1. Globbing
        # Replace template placeholders {x} with wildcard *
        glob_pattern = re.sub(r'\{[^}]+\}', '*', self.template)
        ...
        # 2. Variable Extraction (for itile index)
        # Convert self.template into a regex pattern to capture variables
        ...
        # Drop rows that didn't match the pattern (rare, but possible if glob is more permissive than regex)
        # We align s_files to the valid extracted rows
        valid_mask = extracted.notna().all(axis=1)
        if not valid_mask.all():
            extracted = extracted[valid_mask]
            s_files = s_files[valid_mask]
        ...
        try:
            new_index = [itile.format(**row) for row in records]
        except KeyError as e:
            raise KeyError(
                f"The 'itile' template '{itile}' requires variable {e}, which was not found in the directory template '{self.template}'")
