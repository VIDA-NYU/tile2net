...
def _check_rasterio_im_load(im):
    ...
    Parameters
    ----------
    im : str or rasterio.DatasetReader
...
def prepare_class_gdf(polys, class_name) -> gpd.GeoDataFrame:
    """
...
def prepare_gdf(gdf, **cols) -> gpd.GeoDataFrame:
    """
...
def read_dataframe(src_path, geo=True, cols=None) -> gpd.GeoDataFrame:
    """
	Args:
		src_path:
...
def unary_multi(gdf) -> gpd.GeoDataFrame:
    """
...
def buffer_union(gdf, buff, simp1, simp2) -> gpd.GeoDataFrame:
    """
...
def buffer_union_erode(gdf, buff, erode, simp1, simp2, simp3) -> gpd.GeoDataFrame:
...
def geo2geodf(geo_lst) -> gpd.GeoDataFrame:
    """
...
def merge_dfs(gdf1, gdf2, crs=4326) -> gpd.GeoDataFrame:
    """
...
def create_stats(gdf) -> (pd.DataFrame, gpd.GeoDataFrame):
    """
	creates summary statistics of the polygons in a GeoDataFrame
	Parameters
	----------
	gdf: GeoDataFrame
...
def buff_dfs(gdf, crs) -> gpd.GeoDataFrame:
    """
    ...
    buffersw = buffer_union_erode(dfsw, 0.65, -0.85, 0.1, 0.35, 0.35)
    ...
    bufferrd = buffer_union_erode(dfrd, 0.65, -0.9, 0.1, 0.25, 0.25)
